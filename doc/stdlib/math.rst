...
.. function:: stdm(v, m::Number; corrected::Bool=true)
   ...
   Compute the sample standard deviation of a vector ``v`` with known mean ``m``\ . If ``corrected`` is ``true``\ , then the sum is scaled with ``n-1`` wheares the sum is scaled with ``n`` if ``corrected`` is false``where``\ n = length(x)`.
